...
var nonscaled_xSize = Math.min(window.innerWidth - 20, 1080); // 800 originally
...
for (let i = 0; i<3; i++){
	...
	cloud.scale.x = 0.5 * xRatio;
	cloud.scale.y = 0.5 * yRatio;
	cloud.alpha = 0.5;
...
for (let i = 0; i<3; i++){
	...
	cloud.scale.x = 0.75 * xRatio;
	cloud.scale.y = 0.75 * yRatio;
	cloud.alpha = 0.8;
...
for (let i = 0; i<2; i++){
	...
	cloud.scale.x = xRatio;
	cloud.scale.y = yRatio;
...
starImg.y = 0;
starImg.scale.x = 1.3 * xRatio;
starImg.scale.y = 1.3 * yRatio;
app.stage.addChild(starImg);

// number of star captured in text
var starText = new PIXI.Text("X" + " " + starCount, {fontSize: 35 * xRatio});
...
gameOver.scale.x = 2 * xRatio;
gameOver.scale.y = 2 * yRatio;
...
scoreBar.scale.x = 2 * xRatio;
scoreBar.scale.y = 2 * yRatio;
...
var cloudFigureScaleX = 1.5 * xRatio;
var cloudFigureScaleY = 1.5 * yRatio;
  ...
  var timePass = timeNow - timefromEnd;
    ...
    app.ticker.stop();　　
		...
		var endTextScore = new PIXI.Text("  Score: "+ score, {
			...
			fontWeight: 'bold',
			fill: '0xffffff',
			fontSize: Math.round(90 * xRatio)
		});
		endTextScore.x = app.screen.width / 2 - 203 * xRatio;
